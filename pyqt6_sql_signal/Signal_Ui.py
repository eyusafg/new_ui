...
import threading
from PyQt6.QtWidgets import QMainWindow, QListWidgetItem,QMainWindow,QTreeWidgetItem,QDialog,QMessageBox,QInputDialog
...
import string
from data_QDialog import DeviceInfoDialog
from get_company_QDialog import CustomDialog
...
class IAP(QMainWindow,Ui_IAP):
    def __init__(self,parent = None)-> None:
        ...
        self.Func.table_selected.connect(self.handle_table_selected)
        self.Func.table_selected_received_signal.connect(self.Func.query_company_uid)  # 连接信号
        self.Func.show_table.connect(self.show_machine_data)
    ...
    def closeEvent(self, event):
        win32api.SetFileAttributes(self.file,win32con.FILE_ATTRIBUTE_READONLY | win32con.FILE_ATTRIBUTE_HIDDEN)
        os._exit(0)
    ...
    def get_company_name(self):
        ...
        #     print('company_name', company_name)
        #     # 弹出表名选择对话框
        #     table_type, ok = QInputDialog.getItem(
        #         self,  # 如果要在父窗口居中显示需要传入父窗口对象
        #         "选择机器类型",
        #         "请选择要查询的机器类型:",
        #         ("圆领", "罗纹下摆"),  # 可选项
        #         current=0,  # 默认选中第一个
        ...
        #     if ok and table_type:  # 如果用户点击了确定并且选择了机器类型
        #         print('table_type', table_type)

            self.Func.company_name_received_signal.emit(company_name, serial_number, table_type)  

    def handle_table_selected(self):
        # querydialog = QueryDialog(self)
        # if querydialog.exec() == QDialog.DialogCode.Accepted:
        #     machine_type, sql_query_type, query_column_name, query_condition = querydialog.get_values()
        #     self.Func.table_selected_received_signal.emit(machine_type, sql_query_type, query_column_name, query_condition)  
        ...
        if ok and table_type:  # 如果用户点击了确定并且输入了内容
                print('table_type', table_type)
                self.Func.table_selected_received_signal.emit(table_type)  

    def show_machine_data(self, results):
        DeviceInfoDialog(results, self)
    ...
    # 开启串口
    def Func_Open_Serial(self,*args):# 传信号
    ...
    # 关闭串口
    def Func_Close_Serial(self,*args):
        Serconfig.disable = 1

    # 输入文件
    def FuncDataInput(self, *args): # 导入烧录文件和检查UID合并
        Serconfig.SendCmd = 1
    def FuncEncryptDataInput(self,*args):  # 导入需要加密的文件
        Serconfig.SendCmd = 10
    def FuncSaveParameter(self,*args):  # 保存各种参数到本地
        Serconfig.SendCmd = 9
    def FuncQueryParameter(self,*args):  # 保存各种参数到本地
        Serconfig.SendCmd = 8
    # def Del_Sql_Data(self):  # 清除数据库数据 
    #     Serconfig.SendCmd = 8
    def Del_sql_data(self,*args):
        Serconfig.SendCmd = 7
...
class FuncIAP(QObject):
    ...
    success_signal = pyqtSignal(str)  # 错误信号
    reply_signal = pyqtSignal()  # 错误信号
    show_id_signal = pyqtSignal(str)  # 显示ID信号
    request_company_name_signal = pyqtSignal()  # 请求用户输入公司名
    company_name_received_signal = pyqtSignal(str, str, str)  # 将用户输入的公司名传回
    update_result_signal = pyqtSignal(bool, str)  # 新增：传递更新结果 (成功状态, 消息)
    table_selected = pyqtSignal()
    table_selected_received_signal = pyqtSignal(str)  
    show_table = pyqtSignal(tuple)  
    ...
    def __init__(self, file):
        ...
        global packetstxt
        global indextxt
        self.logger = setup_logging(file)
    ...
    def SerialEnable(self):
        try:
            ...
            self.error_signal.emit("串口已在其他地方打开或者未插入串口")
            print("串口使能失败", e)
    ...
    def get_version(self):
        ...
        # if Vn >= parse_version(Serconfig.comparative_verison) and :      # 大于某个版本的用40000
    ...
    def save_company_uid(self,company_name,serial_number, table_name):  
        # file_name = Bin_Unpacking.Bin_variable.file_name
        # 数据库初始化
        ...
        self.db_file_name = 'syt_machine_data'

        # if 'collar_machine' in file_name:
        #     self.table_name = '圆领'
        # else:
        #     self.table_name = '罗纹下摆'
        self.table_name = table_name
        print('机器类型', self.table_name)
        self.db = DeviceManagementDB(host='192.168.6.239', user='syt', password='03106666', filename=self.db_file_name, table_name=self.table_name)
        ...
        self.logger.info(f"获取硬件版本号：{hardware_version}")
        ...
        # HMI识别码转换
        hmi_code_hex = uid[2] + uid[1] + uid[0] + uid[3] + uid[7] + uid[5:7] + uid[4]
        hmi_code = int(hmi_code_hex,16)
        bias_hex_value = "0x1704"
        decimal_value = int(bias_hex_value, 16)
        HMI_code = hmi_code + decimal_value
        
        self.serial_number = serial_number
        self.HMI_code = HMI_code

        ret = self.db.insert_device(str(company_name), str(uid), str(version_number), str(hardware_version), serial_number, str(HMI_code))
        print('ret', ret)
        if ret == 1:
            self.success_signal.emit("保存成功！")  # 增加显示HMI_code且可以复制功能
        elif ret == -1:
            self.reply_signal.emit()
        else:
            self.error_signal.emit(ret)
        # self.db.backup_table()

    def query_company_uid(self, table_type):
        self.db_file_name = 'syt_machine_data'
        ...
        # self.db.create_table()
        # if query_type == '全量查询':
        #     results = self.db.query_devices()
        # else:
        #     query_condition= query_condition.strip()
        #     results = self.db.query_devices_by_name(query_column_name, str(query_condition))
        # self.show_table.emit(results)
        results = self.db.query_devices()
        self.show_table.emit(results)
    ...
    def get_uid(self):
        ...
        self.logger.info(f"获取软件版本号：{version_number}")
    ...
    def _handle_firmware_flashing(self):
        ...
        self.logger.warning("无法获取UID， 请检查下位机版本")

    def _process_flashing(self, uid):
        '''根据是否收到UID 处理烧录'''

        self.logger.info("固件加密以及匹配UID、机器")
        Bin_Unpacking.Bin_variable.uid = uid
        ret = Bin_Unpacking.encrypt_file()
        if ret == -1:
            self.logger.warning("加密失败！无法烧录！")
            self.error_signal.emit("加密失败！无法烧录！")
        elif ret == -2:
            self.logger.warning("UID不匹配")
            self.error_signal.emit("UID不匹配")
        elif ret == -3:
            self.logger.warning("固件不匹配， 请重新选择")
            self.error_signal.emit("固件不匹配， 请重新选择")
        elif ret:
    ...
    def HaveAPPUpdata(self):
        ...
        self.logger.info(f'烧录文件名为：{Bin_Unpacking.Bin_variable.file_name}')
        self.logger.info("开始烧录")
        ...
        self.logger.info(f"烧录完成， 版本号：{version_number}")
